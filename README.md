...
[Click here to finish setting up your workspace!](https://cloud.nx.app/connect/Jxpco9VN6a)
...
**Important:** The repository previously contained a tracked `apps/smart-academy/.env` with plaintext credentials. Treat any exposed credentials as compromised and rotate them immediately (revoke and create new passwords/keys). Do not commit real credentials — commit only `apps/smart-academy/.env.example` or store secrets in a secure manager.
